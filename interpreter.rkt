...
(define mstate
    (lambda (ptree state)
        (cond
            ...
            ((return-op? ptree) ; ptree == (((return value) ...)
                (mstate (next-statement ptree) (return-op ptree state))
            )
            ((while-op? ptree) ; ptree == ((while cond body) ...)
                (mstate (next-statement ptree) (while-op ptree state))
            )
            ((assign-op? ptree) ; ptree == ((= name newvalue) ...)
                (mstate (next-statement ptree) (assign-op ptree state))
            )
            ((declare-op? ptree) ; ptree == ((var name) ...)
                (mstate (next-statement ptree) (declare-op ptree state))
            )
            ((declare-assign-op? ptree) ; ptree == ((var name value) ...)
                (mstate (next-statement ptree) (declare-assign-op ptree state))
            )
            ((if-op? ptree) ; ptree == ((if cond body) ...)
                (mstate (next-statement ptree) (if-op ptree state))
            )
            ((if-else-op? ptree) ; ptree == ((if cond body else-body) ...)
                (mstate (next-statement ptree) (if-else-op ptree state))
            )
            (else
